...
const TaskList = ({ tasksData, filter, onFinishEditing, onChangeDescription, onToggleProperty, onDelete }) => {
  const tasks = tasksData.map((taskData) => {
    ...
    return (
      <li className={classNames} key={taskData.id}>
        <Task {...taskData} onToggleProperty={onToggleProperty} onDelete={onDelete} />
        {inputField}
      </li>
    );
  });
